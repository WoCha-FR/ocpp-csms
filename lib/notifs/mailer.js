...
export default new (class eMailer {
  ...
  async initMailer() {
    ...
    if (utils.config().email.mode === 'sendmail') {
      this.transporter = nodemailer.createTransport(utils.config().email.sendmail)
    }
    // Setup SMTP transporter
    else if (utils.config().email.mode === 'smtp') {
      this.transporter = nodemailer.createTransport(utils.config().email.smtp)
    }
    // Setup service transporter
    else if (utils.config().email.mode === 'service') {
      this.transporter = nodemailer.createTransport(utils.config().email.service)
    }
    ...
    utils.event.on('userCreated', (usermail, userpwd) => {
      this.log.log('Event userCreated received in mailer')
      const replaceStr = {
        '{webname}': utils.config().http.title,
        '{weburl}': utils.config().http.host,
        '{user_mail}': usermail,
        '{user_pwd}': userpwd,
      }
      const userData = {
