...
export default async app => {
  ...
  if (utils.config().authGoogle.enabled === true) {
    passport.use(new GoogleStrategy({
        clientID: utils.config().authGoogle.clientID,
        clientSecret: utils.config().authGoogle.clientSecret,
        callbackURL: utils.config().http.host + '/auth/google/callback/',
        scope: ['email','profile'],
        state: true
      },
  ...
  let loginSchema = {
    username: { trim: true, escape: true, normalizeEmail: {options: {gmail_remove_dots: false}}, isEmail: {options: {allow_ip_domain: false}}},
    password: { trim: true, escape: true}
  }
  // Sanitize and Authenticate
  app.post('/auth/login', checkSchema(loginSchema), passport.authenticate('local', {
