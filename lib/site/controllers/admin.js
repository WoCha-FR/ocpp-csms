...
const AdminController = {
  ...
  UpdSiteSchema: {
    ...
    scodp: {
      optional : { options: { values: 'falsy' } },
      trim: true, escape: true, isPostalCode: { options: 'FR' }
    },
    sville: {
      optional : { options: { values: 'falsy' } },
      trim: true, escape: true, isLength: { options: { max: 150 } },
  ...
  updSite: async (req, res) => {
    ...
    if (result.isEmpty()) {
      ...
      // Validation False
      req.session.ocppcsms.ocppErrors = { ocppInput: result.array() }
      res.redirect('/site/edit/' + req.params.sitepk)
      return undefined
    }
  },
  SiteId: {
    siteId: {
      isInt: { options: {min: 2} }, trim: true, escape: true,
    }
  },
