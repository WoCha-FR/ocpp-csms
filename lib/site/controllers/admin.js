...
const AdminController = {
  ...
  SiteSchema: {
    ...
    saddr: { optional: { options: { values: 'falsy' } }, trim: true, escape: true, isLength: { options: { max: 255 } } },
    scodp: { optional: { options: { values: 'falsy' } }, trim: true, escape: true, isPostalCode: { options: 'any' } },
    sville: {
  ...
  addSite: async (req, res) => {
    ...
    if (result.isEmpty()) {
      ...
      let params = { name: data.sname, address: data.saddr, codp: data.scodp, ville: data.sville }
      // Add Site
  ...
  UpdSiteSchema: {
    sitepk: { isInt: true, trim: true, escape: true },
    sname: {
      trim: true,
      escape: true,
      isLength: { options: { min: 5, max: 50 } },
  ...
  updSite: async (req, res) => {
    ...
    if (result.isEmpty()) {
      let data = matchedData(req)
      let params = { sid: data.sitepk, name: data.sname, address: data.saddr, codp: data.scodp, ville: data.sville }
  ...
  UserAddSchema: {
    ...
    password: { trim: true, escape: true, isString: true, isStrongPassword: { options: { minLength: 8, maxLength: 50 } } },
    uname: {
      trim: true,
      escape: true,
      isLength: { options: { min: 5, max: 50 } },
    ...
    ulevel: { optional: { options: { values: 'falsy' } }, trim: true, escape: true, isInt: true, default: null },
  },
  addUser: async (req, res) => {
    ...
    if (result.isEmpty()) {
      ...
      let params = { umail: data.usermail, uname: data.uname, upass: crypted, owner: data.ulevel, admin: null }
      // Set Admin if needed
      if (data.ulevel == 0) {
        params.admin = 1
        params.owner = null
  ...
  UserUpdSchema: {
    userpk: { isInt: true, trim: true, escape: true },
    usermail: {
      normalizeEmail: { options: { gmail_remove_dots: false } },
      isEmail: { options: { allow_ip_domain: false } },
    },
  ...
  updUser: async (req, res) => {
    ...
    if (result.isEmpty()) {
      ...
      let params = { uid: data.userpk, umail: data.usermail, uname: data.uname, owner: data.ulevel, admin: null }
      if (data.password) {
        const crypted = await bcrypt.hash(data.password, 10)
  ...
  ConfOcppSchema: {
    newcfg_pk: { trim: true, escape: true, isInt: { options: { min: 1, max: 20 } } },
    newenabled: { isBoolean: true, toBoolean: true },
    newvalue: {
      exists: { if: body('newenabled').equals('true') },
      trim: true,
      escape: true,
  ...
  updateConfigOcpp: async (req, res) => {
    ...
    if (result.isEmpty()) {
      let data = matchedData(req)
      let params = { cfg_pk: data.newcfg_pk, value: data.newvalue, enabled: data.newenabled === true ? 1 : 0 }
  ...
  /* Pendings Station */
  PendingSchema: { ocppName: { trim: true, escape: true }, action: { isIn: { options: [['Accepted', 'Rejected']] } } },
  ...
  registerPending: async (req, res) => {
    ...
    if (result.isEmpty()) {
      const data = matchedData(req)
      const params = { ocppid: data.ocppName, cbname: data.ocppName, regstatus: data.action }
      // Ajout en DB
