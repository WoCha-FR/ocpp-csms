...
const RfidController = {
  ...
  formUpdRfidTag: async (req, res) => {
    ...
    if (result.isEmpty()) {
      ...
      let params = { rf_pk: data.elemId, sid: null }
  ...
  RfidUsersOfSite: {
    rfsite: { trim: true, escape: true, isInt: true, optional: { options: { values: 'falsy' } }, default: null },
  },
  ...
  formUpdOwnerRfidTag: async (req, res) => {
    ...
    if (result.isEmpty()) {
      let data = matchedData(req)
      let params = { rf_pk: data.elemId, sid: req.session.passport.user.ownedSite }
      let response = await sqlite.getRfidTagInfosById(params)
  ...
  RfidTagAddSchema: {
    ...
    rfsite: { optional: { options: { values: 'falsy' } }, isInt: true, trim: true, escape: true, default: null },
    rfname: { trim: true, escape: true, isLength: { options: { min: 1, max: 50 } } },
    rfblocked: { trim: true, escape: true, isInt: { options: { min: 0, max: 1 } } },
    rfexpire: { optional: { options: { values: 'falsy' } }, trim: true, escape: true, default: null, isInt: true },
    rfuser: { optional: { options: { values: 'falsy' } }, trim: true, escape: true, default: null, isInt: true },
  },
  addRfidTag: async (req, res) => {
    const result = validationResult(req)
    if (result.isEmpty()) {
      let data = matchedData(req)
      let params = { idtag: data.rftag, rf_name: data.rfname, user: data.rfuser, expire: data.expire, blocked: data.blocked }
  ...
  RfidTagUpdSchema: {
    rfpk: { isInt: true, trim: true, escape: true },
    rftag: {
      trim: true,
      escape: true,
      matches: { options: /^[a-zA-Z0-9]*$/ },
  ...
  delRfidTag: async (req, res) => {
    ...
    if (result.isEmpty()) {
      let data = matchedData(req)
      let params = { rf_pk: data.elemId, isAdmin: req.session.passport.user.isAdmin }
