...
const ChargerController = {
  ...
  UpdChargerSchema: {
    ...
    cbname: { trim: true, escape: true, matches: { options: /^[a-zA-ZÀ-Ÿ0-9-_ ]*$/ }, isLength: { options: { min: 5 } } },
    cbmode: { isInt: { options: { min: 0, max: 2 } }, trim: true, escape: true },
    cbid: { trim: true, escape: true },
    cbpwd: { optional: { options: { values: 'falsy' } }, trim: true, escape: true, isLength: { options: { min: 5 } } },
  },
  ...
  ConfigSchema: { ocppName: { trim: true, escape: true }, key: { trim: true, escape: true }, value: { trim: true } },
  ...
  RebootSchema: { ocppName: { trim: true, escape: true }, ResetType: { isIn: { options: [['Soft', 'Hard']] } } },
  ...
  setAvailability: async (req, res) => {
    ...
    if (result.isEmpty()) {
      const data = matchedData(req)
      const params = { ocppName: data.ocppName, cn_id: data.connectorId, type: data.AvailabilityType }
      // Ckeck if station is owned by this user
      if (req.session.passport.user.ownedSite) {
        let valid = await sqlite.isOwnerOfChargeBox(data.ocppName, req.session.passport.user.ownedSite)
        if (valid === false) {
  ...
  TriggerSchema: {
    ocppName: { trim: true, escape: true },
    requestedMessage: { isIn: { options: [['BootNotification', 'Heartbeat', 'StatusNotification', 'MeterValues']] } },
    connectorId: {
      optional: { options: { values: 'falsy' } },
      trim: true,
      escape: true,
  ...
  triggerMessage: async (req, res) => {
    ...
    if (result.isEmpty()) {
      const data = matchedData(req)
      const params = { ocppName: data.ocppName, message: data.requestedMessage, connid: data.connectorId }
      // Ckeck if station is owned by this user
      if (req.session.passport.user.ownedSite) {
        let valid = await sqlite.isOwnerOfChargeBox(data.ocppName, req.session.passport.user.ownedSite)
        if (valid === false) {
  ...
  TransactionSchema: { ocppName: { trim: true, escape: true }, transId: { isInt: true, trim: true, escape: true } },
