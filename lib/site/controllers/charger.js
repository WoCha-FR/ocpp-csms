...
const ChargerController = {
  ...
  getSocketPage: async (req, res) => {
    ...
    // Last Transaction by Connector
    const res3 = await sqlite.getChargeBoxConsLastTransac(data.ocppName, req.session.passport.user.ownedSite)
    if (res3.error) {
      req.session.ocppcsms.ocppErrors = { sqlite: [{sqlerror: `${res3.error}`}] }
    }
    // Render
  ...
  ajaxGetConsLastTransac: async (req, res) => {
    const data = matchedData(req)
    const result = await sqlite.getChargeBoxConsLastTransac(data.ocppName, req.session.passport.user.ownedSite)
    if (!result) {
      res.status(404).send('Not Found')
    }
    if (result.error) {
  ...
  //TODO: Add Filter on Request type
  LogsSchema: {
  ...
  RemoteStartTrans: async(req, res) => {
    ...
    if (result.isEmpty()) {
      ...
      const params = {
        ocppName: data.ocppName,
        cn_id: data.connectorId,
        //TODO: Identify user who request remote start
        idTag: 'FromOcppCsms'
      }
      // Ckeck if station is owned by this user
      if (req.session.passport.user.ownedSite) {
