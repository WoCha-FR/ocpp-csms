...
const ChargerController = {
  getMainPage: async (req, res) => {
    ...
    const res2 = await sqlite.getChargeBoxBootInfo(
      data.ocppName,
      req.session.passport.user.ownedSite
    )
    if (res2 && res2.error) {
      req.session.ocppcsms.ocppErrors = { sqlite: [{ sqlerror: `${res2.error}` }] }
    }
    // Get ChargeBox Connectors
    const res3 = await sqlite.getChargeBoxConnectors(
      data.ocppName,
      req.session.passport.user.ownedSite
    )
    if (res3.error) {
  ...
  ajaxGetChargerStatus: async (req, res) => {
    const data = matchedData(req)
    const result = await sqlite.ajaxGetChargeurStatus(
      data.ocppName,
      req.session.passport.user.ownedSite
    )
    if (!result) {
  ...
  ajaxGetChargerConnsStatus: async (req, res) => {
    const data = matchedData(req)
    const result = await sqlite.ajaxGetChargeurConnsStatus(
      data.ocppName,
      req.session.passport.user.ownedSite
    )
    if (!result) {
  ...
  UpdChargerSchema: {
    ...
    cbmode: { isInt: { options: { min: 0, max: 2 } }, trim: true, escape: true },
    cbid: { trim: true, escape: true },
    cbpwd: {
  ...
  updCharger: async (req, res) => {
    ...
    if (result.isEmpty()) {
      ...
      let params = { id: data.ocppName, name: data.cbname, mode: data.cbmode }
  ...
  UpdConnPersoSchema: {
    ocppName: { trim: true, escape: true },
    connectorId: { isInt: { options: { min: 0, max: 9 } }, trim: true, escape: true },
    cmaxp: { isInt: { options: { min: 1, max: 400 } }, trim: true, escape: true },
    cformat: { isInt: { options: { min: 1, max: 2 } }, trim: true, escape: true },
    ctype: { isInt: { options: { min: 1, max: 9 } }, trim: true, escape: true },
    cname: {
      optional: { options: { values: 'falsy' } },
      trim: true,
      escape: true,
  ...
  getSocketPage: async (req, res) => {
    ...
    const res2 = await sqlite.getChargeBoxConsAndLastTransac(
      data.ocppName,
      req.session.passport.user.ownedSite
    )
    if (res2.error) {
  ...
  ajaxGetConsLastTransac: async (req, res) => {
    const data = matchedData(req)
    const result = await sqlite.ajaxGetChargeBoxConsLastTransac(
      data.ocppName,
      req.session.passport.user.ownedSite
    )
    if (!result) {
  ...
  LogsSchema: {
    ocppName: { trim: true, escape: true },
    start: { isInt: true, trim: true, escape: true },
    stop: { isInt: true, trim: true, escape: true },
  },
  ...
  RebootSchema: {
    ocppName: { trim: true, escape: true },
    ResetType: { isIn: { options: [['Soft', 'Hard']] } },
  },
  rebootStation: async (req, res) => {
    ...
    if (result.isEmpty()) {
      const data = matchedData(req)
      const params = { ocppName: data.ocppName, rebootType: data.ResetType }
      // Ckeck if station is owned by this user
      if (req.session.passport.user.ownedSite) {
        let valid = await sqlite.isOwnerOfChargeBox(
          data.ocppName,
          req.session.passport.user.ownedSite
        )
        if (valid === false) {
  ...
  AvailabilitySchema: {
    ocppName: { trim: true, escape: true },
    connectorId: { isInt: { options: { min: 0, max: 9 } }, trim: true, escape: true },
    AvailabilityType: { isIn: { options: [['Operative', 'Inoperative']] } },
  },
  ...
  TriggerSchema: {
    ocppName: { trim: true, escape: true },
    requestedMessage: {
      isIn: { options: [['BootNotification', 'Heartbeat', 'StatusNotification', 'MeterValues']] },
    },
    connectorId: {
      optional: { options: { values: 'falsy' } },
      trim: true,
  ...
  ConnTriggerSchema: {
    ocppName: { trim: true, escape: true },
    requestedMessage: { isIn: { options: [['StatusNotification', 'MeterValues']] } },
    connectorId: { trim: true, escape: true, isInt: { options: { min: 0, max: 9 } } },
  },
  ...
  ConnectorSchema: {
    ocppName: { trim: true, escape: true },
    connectorId: { isInt: { options: { min: 1, max: 9 } }, trim: true, escape: true },
  },
  unlockConnector: async (req, res) => {
    ...
    if (result.isEmpty()) {
      const data = matchedData(req)
      const params = { ocppName: data.ocppName, cn_id: data.connectorId }
      // Ckeck if station is owned by this user
      if (req.session.passport.user.ownedSite) {
        let valid = await sqlite.isOwnerOfChargeBox(
          data.ocppName,
  ...
  GrantedRemoteStartTrans: async (req, res) => {
    ...
    if (result.isEmpty()) {
      ...
      if (req.session.passport.user.ownedSite !== null) {
        const valid = await sqlite.isOwnerOfChargeBox(
          data.ocppName,
          req.session.passport.user.ownedSite
        )
        if (valid === false) {
      ...
      const adminfo = await sqlite.getOwnerOrAdminIdTag(req.session.passport.user.ownedSite)
      if (adminfo.error) {
        req.session.ocppcsms.ocppErrors = { sqlite: [{ sqlerror: `${adminfo.error}` }] }
        res.redirect(req.get('Referrer') || '/charger/' + data.ocppName + '/connectors')
      }
      // Send ocpp request
      const params = { ocppName: data.ocppName, cn_id: data.connectorId, idTag: adminfo.idtag }
  ...
  UserRemoteStartTrans: async (req, res) => {
    ...
    if (result.isEmpty()) {
      ...
      const uvalid = await sqlite.isUserOfChargeBox(
        data.ocppName,
        req.session.passport.user.user_pk
      )
      if (uvalid === false) {
        res.redirect('/forbidden')
      }
      const sqlparams = { cbid: data.ocppName, uid: req.session.passport.user.user_pk }
      // Get user idTag
      const taginfo = await sqlite.getUserIdTag(sqlparams)
      if (taginfo.error) {
        req.session.ocppcsms.ocppErrors = { sqlite: [{ sqlerror: `${taginfo.error}` }] }
        res.redirect(req.get('Referrer') || '/connectors/' + data.ocppName)
      }
      // Send ocpp request
      const params = { ocppName: data.ocppName, cn_id: data.connectorId, idTag: taginfo.idtag }
      const response = await csms.RemoteStartTransaction(params)
      req.session.ocppcsms.ocppResult = response
      res.redirect(req.get('Referrer') || '/connectors/' + data.ocppName)
    } else {
  ...
  UserRemoteStopTrans: async (req, res) => {
    // Validator
    const result = validationResult(req)
    if (result.isEmpty()) {
      const data = matchedData(req)
      const params = { ocppName: data.ocppName, transacId: data.transId }
      // Check if this station is auhorized for this user
      const uvalid = await sqlite.isUserOfChargeBox(
        data.ocppName,
        req.session.passport.user.user_pk
      ...
      req.session.ocppcsms.ocppResult = response
      res.redirect(req.get('Referrer') || '/connectors/' + params.ocppName)
    } else {
      req.session.ocppcsms.ocppErrors = { ocppInput: result.array() }
      res.redirect(req.get('Referrer') || '/connectors/' + req.params.ocppName)
    }
  ...
  TransactionSchema: {
    ocppName: { trim: true, escape: true },
    transId: { isInt: true, trim: true, escape: true },
  },
  RemoteStopTrans: async (req, res) => {
    ...
    if (result.isEmpty()) {
      ...
      if (
        ...
        req.session.passport.user.ownedSite === false
      ) {
        let uvalid = await sqlite.isUserOfChargeBox(
          data.ocppName,
          req.session.passport.user.user_pk
        )
        if (uvalid === false) {
