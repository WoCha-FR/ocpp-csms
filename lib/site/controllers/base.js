...
const BaseController = {
  ...
  updMesNotifications: async(req, res) => {
    ...
    if (result.isEmpty()) {
      ...
      let params = {
        uid: req.session.passport.user.user_pk,
        lng: data.lang,
        eml: data.emailNotifs,
        psh: data.pushNotifs,
        pshu: data.pushUser || null,
        psht: data.pushToken || null,
        cbon: data.cbOnline,
        cboff: data.cbOffline,
        conf: data.conFaulted,
        cona: data.conAvailable,
        conu: data.conUnavailable,
        chgd: data.chgStart,
        chgf: data.chgStop,
        chge: data.chgEvents
      }
  ...
  NotifsSchema: {
    ...
    pushUser: {
      trim: true, escape: true, optional: true,
    ...
    chgEvents: { optional: true, default: 0, isIn: { options: [[0, 1]] }}
  },
  OcppidSchema: {
