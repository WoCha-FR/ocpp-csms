...
export default new (class CSMS {
  constructor() {
    this.log = logger.child({ childLabel: 'OCPP' })
  ...
  async initServer() {
    // Logger
    this.log.info('Initializing CSMS OCPP Server...')
    // Init Pending DB
    this.PendingCB = await JSONFilePreset(utils.rootdir() + '/datas/pendings.json', {
      pendings: [],
    })
    // Server Auth
    ...
    this.server.on('client', async (client) => {
      ...
      client.handle('StatusNotification', async ({ params, reply }) => {
        ...
        if (params.connectorId > 0) {
          this.log.debug(
            `StatusNotification from ${client.identity} - connector ${params.connectorId}`
          )
          // Emit event
      ...
      client.handle('StopTransaction', async ({ params, reply }) => {
        this.log.debug(
          `StopTransaction from ${client.identity} - transaction ${params.transactionId}`
        )
        const expire = new Date(Date.now()).toISOString()
      ...
      client.handle('MeterValues', async ({ params, reply }) => {
        ...
        if (meters.length > 1) {
          let e1 = meters.find(
            (s) => s.sampledValue[0].measurand == 'Energy.Active.Import.Register'
          )
          if (e1) {
            // Get Unit
          ...
          let e1 = meters[0].sampledValue.find(
        ...
        if (params.transactionId !== undefined && params.transactionId !== 0) {
          ...
          values.ampl1 = null
          if (meters.length > 1) {
            let cur1 = meters.find(
              (s) =>
                s.sampledValue[0].measurand == 'Current.Import' && s.sampledValue[0].phase == 'L1'
            )
            if (cur1) {
              values.ampl1 = parseFloat(cur1.sampledValue[0].value)
            }
          } else {
            let cur1 = meters[0].sampledValue.find(
              (s) => s.measurand == 'Current.Import' && s.phase == 'L1'
            )
            if (cur1) {
          ...
          values.ampl2 = null
          if (meters.length > 1) {
            let cur2 = meters.find(
              (s) =>
                s.sampledValue[0].measurand == 'Current.Import' && s.sampledValue[0].phase == 'L2'
            )
            if (cur2) {
              values.ampl1 = parseFloat(cur2.sampledValue[0].value)
            }
          } else {
            let cur2 = meters[0].sampledValue.find(
              (s) => s.measurand == 'Current.Import' && s.phase == 'L2'
            )
            if (cur2) {
          ...
          values.ampl3 = null
          if (meters.length > 1) {
            let cur3 = meters.find(
              (s) =>
                s.sampledValue[0].measurand == 'Current.Import' && s.sampledValue[0].phase == 'L3'
            )
            if (cur3) {
              values.ampl3 = parseFloat(cur3.sampledValue[0].value)
            }
          } else {
            let cur3 = meters[0].sampledValue.find(
              (s) => s.measurand == 'Current.Import' && s.phase == 'L3'
            )
            if (cur3) {
      ...
      client.handle('DiagnosticsStatusNotification', async ({ params }) => {
        this.log.debug(
          `DiagnosticsStatusNotification from ${client.identity} - status ${params.status}`
        )
        // Emit event
  ...
  async TriggerMessage(params) {
    const client = this.ocppClients.get(params.ocppName)
    if (client) {
      let payload = { requestedMessage: params.message }
      if (parseInt(params.connid) > 0) {
        payload['connectorId'] = parseInt(params.connid)
      }
      try {
        const response = await client.call('TriggerMessage', payload)
  ...
  async UnlockConnector(params) {
    ...
    if (client) {
      try {
        const response = await client.call('UnlockConnector', {
          connectorId: parseInt(params.cn_id),
        })
    ...
    if (client) {
      try {
        const response = await client.call('RemoteStopTransaction', {
          transactionId: parseInt(params.transacId),
        })
  ...
  async GetDiagnostics(params) {
    ...
    if (client) {
      try {
        ...
        return { fileName: err.rpcErrorMessage }
      }
    } else {
  ...
  // Start function
  async Start() {
    await this.server.listen(conf.get('ocpp.portws'), conf.get('ocpp.listenip'))
    this.log.info(`CSMS started on ${conf.get('ocpp.listenip')}:${conf.get('ocpp.portws')}`)
  }
  // Stop Function
  async Stop() {
    await this.server.close({ code: 1012, awaitPending: true })
    this.log.info('CSMS stopped')
  }
  // Triggers
