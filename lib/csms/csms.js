...
export default new class CSMS {
    ...
    this.server.on('client', async (client) => {
      ...
      client.handle('BootNotification', async ({params, reply}) => {
        ...
        if (cbox) {
          ...
          // TODO - Automatic configuration & request from BDD
        } else {
      ...
      client.handle('StatusNotification', async ({params, reply}) => {
        ...
        // Send response
        reply({})
        // Status Preparing & Charger in autostart mode
    ...
    if (client) {
      try {
        ...
        if (response.status === 'Accepted') {
          const result = await sqlite.setChargeBoxConfigKey(params)
          if (result.error) {
  ...
  async GetDiagnostics(params) {
    ...
    if (client) {
      try {
        ...
        this.log.debug('GetDiagnostics Request: ' + JSON.stringify(response))
        return response
      } catch ( err ) {
  ...
  // Start function
  async Start () {
    await this.server.listen(utils.config().ocpp.port, '0.0.0.0')
    this.log.log('CSMS started at port ' + utils.config().ocpp.port)
  }
  // Stop Function
  async Stop () {
