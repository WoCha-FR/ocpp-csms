    ...
    this.server.on('client', async (client) => {
      ...
      client.handle('Authorize', async({params}) => {
        ...
        if (params.idTag === 'FromOcppCsms') {
          ...
          let result = await CSMSDB.getAuthorizeResponse(client.identity, params.idTag)
      ...
      client.handle('StartTransaction', async ({params, reply}) => {
        ...
        let transId = await CSMSDB.generateTransactionId()
        if (transId === false) {
        ...
        let result
        let expiry = new Date(Date.now() + 86400000).toISOString()
        //TODO: IDTAG for Identify user who request remote start
        if (params.idTag === 'FromOcppCsms') {
          result = { status: 'Accepted', expiryDate: expiry }
        } else if (await CSMSDB.isChargerAutonomous(client.identity)) {
          result = { status: 'Accepted', expiryDate: expiry }
        } else {
          result = await CSMSDB.getAuthorizeResponse(client.identity, params.idTag)
      ...
      client.handle('MeterValues', async ({params, reply}) => {
        ...
        if (params.transactionId !== undefined && params.transactionId !== 0) {
          ...
          /* TODO: Valid new transaction store
          let transacDatas = {
            tpk: params.transactionId,
