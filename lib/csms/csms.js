    ...
    this.server.on('client', async (client) => {
      ...
      client.handle('StatusNotification', async ({params, reply}) => {
        ...
        if (params.connectorId > 0) {
          ...
          if (params.status === 'Preparing') {
            let res = await sqlite.isChargerAutoOCPP(client.identity)
            if (res) {
      ...
      client.handle('StartTransaction', async ({params, reply}) => {
        ...
        if (transId === false) {
          ...
          reply({idTagInfo: { status: 'Invalid' }, transactionId: Math.floor(Date.now() / 1000)})
          return
        }
        ...
        } else if (await sqlite.isChargerAutonomous(client.identity)) {
          result = { status: 'Accepted', expiryDate: expiry }
        } else {
        ...
        // Set in transacParams
        transacParams.status = result.status
        ...
        // Response to charger
        this.log.info(`Transaction ${transId} in ${client.identity}: ${result.status}`)
        reply({idTagInfo: result , transactionId: transId})
        // Initialize TransactionData in DB
        let transacDatas = {
          tpk: transId,
          ts: (Date.parse(new Date(params.timestamp)) / 1000)
        }
        insert = await sqlite.initTransactionData(transacDatas)
        if (insert === false) {
      ...
      client.handle('StopTransaction', async ({params, reply}) => {
        ...
        let insert = await sqlite.addTransactionData(transacDatas)
        if (insert === false) {
          this.log.warn(`Transaction ${params.transactionId} - addTransactionData Error`)
        }
      })
      ///////////////////////////////////////////////////////////////////
      ...
      client.handle('MeterValues', async ({params, reply}) => {
        ...
        if (params.transactionId !== undefined && params.transactionId !== 0) {
          ...
          // Store in DB : Transaction meter values
          await sqlite.updTransactionMeterValues(values)
          // Store in DB : Transaction data values
          let transacDatas = {
            tpk: params.transactionId,
            ts: (Date.parse(new Date(params.meterValue[0].timestamp)) / 1000),
            energy: values.energy,
            power: values.power
          }
          await sqlite.addTransactionData(transacDatas)
        }
      })
      ///////////////////////////////////////////////////////////////////
