...
export default new class ReportsDB {
  ...
  async getTransacList(params) {
    ...
    if (params.sid) {
      // Owner
      query = 'SELECT `CB`.`cb_name`,`CS`.`cname`,`CS`.`connector_id`,`RF`.`rf_name`,`SU`.`uname`,`U`.`usermail`,' +
      ...
      // Admin
      query = 'SELECT `S`.`s_name`,`CB`.`cb_name`,`CS`.`cname`,`CS`.`connector_id`,`RF`.`rf_name`,`U`.`name` AS `uname`,`U`.`usermail`,' +
        '`T`.`t_pk`,`T`.`upk`,`T`.`rfpk`,`T`.`IdToken`,`T`.`tsStart`,`T`.`tsStop`,`T`.`tsStatus`,`T`.`tsEnergy`,`T`.`tsReason` ' +
        'FROM `transactions` `T` ' +
        'LEFT JOIN `connector_status` `CS` ON (`T`.`con_pk`=`CS`.`pk`) ' +
        'LEFT JOIN `charge_box` `CB` ON (`CS`.`cb_pk`=`CB`.`cb_pk`) ' +
  ...
  async getTransacAuthList(params) {
    ...
    if (params.sid) {
      // Owner
      query = 'SELECT `CB`.`cb_name`,`A`.`idtag`,`A`.`ts`,`A`.`resp`,`A`.`method` ' +
        'FROM `idtags_logs` `A` ' +
      ...
      // Admin
      query = 'SELECT `S`.`s_name`,`CB`.`cb_name`,`A`.`idtag`,`A`.`ts`,`A`.`resp`,`A`.`method` ' +
        'FROM `idtags_logs` `A` ' +
        'LEFT JOIN `charge_box` `CB` ON (`A`.`cb_pk`=`CB`.`cb_pk`) ' +
        'LEFT JOIN `sites` `S` ON (`CB`.`site`=`S`.`s_pk`) ' +
  ...
  async getTransacData(params) {
    const query = 'SELECT `S`.`s_name`,`CB`.`cb_name`,`CS`.`cname`,`CS`.`connector_id`,`RF`.`rf_name`,`U`.`name` AS `uname`,' +
      '`T`.`t_pk`,`T`.`upk`,`T`.`rfpk`,`T`.`IdToken`,`T`.`tsStart`,`T`.`tsStop`,`T`.`tsStatus`,`T`.`tsEnergy`,`T`.`tsReason` ' +
      'FROM `transactions` `T` ' +
      'LEFT JOIN `connector_status` `CS` ON (`T`.`con_pk`=`CS`.`pk`) ' +
      'LEFT JOIN `charge_box` `CB` ON (`CS`.`cb_pk`=`CB`.`cb_pk`) ' +
