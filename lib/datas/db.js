...
export default new class sqlite {
  ...
  async getSiteUsers(sid) {
    let query = 'SELECT `S`.`pk`, `S`.`uname`, `U`.`usermail` FROM sites_users AS S ' +
  ...
  async updRfidTag(params) {
    let query = 'UPDATE rfid_tags SET `site`=@site,`idtag`=@idtag,`rf_name`=@rf_name,`user`=@user,`expire`=@expire,' +
