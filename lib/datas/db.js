...
export default new (class sqlite {
  ...
  async initDB() {
    ...
    fs.stat(dbFile, async function (err) {
      ...
      if (err && err.code === "ENOENT") {
        ...
        const struct = fs.readFileSync(path + "db.sql", {
          encoding: "utf8",
          flag: "r",
        });
        try {
    ...
    // Migration needed ?
    try {
      const dbversion = this.db
        .prepare("SELECT `version` FROM `schema_version` WHERE `schema_pk`=1")
        .pluck()
        .get();
  ...
  async updSiteUser(params) {
    const query =
      "UPDATE `sites_users` SET `uname`=@uname WHERE `pk`=@pk AND `spk`=@spk";
  ...
  async addSiteUser(params) {
    const query =
      "INSERT INTO `sites_users` (`spk`,`upk`,`uname`) VALUES (@spk,@upk,@uname)";
  ...
  async getOcppDefaultConf() {
    const query =
      "SELECT * FROM `ocpp_default_conf` ORDER BY `enabled` DESC, `key` ASC";
  ...
  async getChargeBoxByName(ocppid, sid = null) {
    ...
    if (sid === null) {
      ...
      query =
        "SELECT * FROM `charge_box` WHERE `site`=" + sid + " AND `cb_id`=?";
    }
  ...
  async updChargeBoxNameMode(params, sid = null) {
    ...
    if (sid === null) {
      query =
        "UPDATE `charge_box` SET `cb_name`=@name, `cb_mode`=@mode WHERE `cb_id`=@id";
    } else {
      query =
  ...
  async getOwnerOrAdminIdTag(sid) {
    ...
    if (sid === null) {
      ...
      query =
        "SELECT `idtag` FROM `rfid_tags` WHERE `site`=" +
        sid +
        " AND `isparent`=1";
    }
  ...
  async isOwnerOfChargeBox(ocppid, sid) {
    const query =
      "SELECT `cb_pk` FROM `charge_box` WHERE `cb_id`=? AND `site`=?";
    try {
      const stmt = this.db.prepare(query);
  ...
  async getChargeBoxConsLastTransac(ocppid, sid = null) {
    ...
    return await this.sqliteAll(
      "getChargeBoxConsLastTransac",
      query,
      ocppid,
      true,
    );
  ...
  async getChargeBoxConsAndLastTransac(ocppid, sid = null) {
    ...
    return await this.sqliteAll(
      "getChargeBoxConsAndLastTransac",
      query,
      ocppid,
      true,
    );
  ...
  async ajaxGetChargeBoxConsLastTransac(ocppid, sid = null) {
    ...
    return await this.sqliteAll(
      "ajaxGetChargeBoxConsLastTransac",
      query,
      ocppid,
      true,
    );
  ...
  async setAllChargeBoxOffline() {
    try {
      const info = this.db
        .prepare("UPDATE `charge_box` SET `online`=0 WHERE `online`=1")
        .run();
      this.log.debug(
        "setAllChargeBoxOffline: " + info.changes + " set offline",
      );
    } catch (err) {
