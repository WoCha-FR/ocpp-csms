...
class sqlite {
  constructor() {
    this.db
    this.log = logger.child({ childLabel: 'DB' })
  }
  ...
  async initDB() {
    ...
    try {
      ...
      // File doesn't exist - create DB
      if (err.code === 'ENOENT') {
        // Create DB
        const tempDB = new Database(dbFile)
        // Create structure
        const struct = readFileSync(sqlPath + 'initial.sql', { encoding: 'utf8', flag: 'r' })
        try {
          ...
          this.log.error('Error creating database : ' + err)
  ...
  getUserCredentials(usermail) {
    // Passport login
    const query = 'SELECT `user_pk`,`usermail`,`password`,`name`,`isAdmin`,`ownedSite` ' + 'FROM `users` WHERE `usermail`=?'
    try {
  ...
  getUserGoogleCredentials(gglid) {
    const query = 'SELECT `user_pk`,`usermail`,`password`,`name`,`isAdmin`,`ownedSite` ' + 'FROM `users` WHERE `gglid`=?'
    try {
      const stmt = this.db.prepare(query)
  ...
  async addSite(params) {
    const query = 'INSERT INTO `sites` (`s_name`,`s_address`,`s_codp`,`s_ville`) VALUES ' + '(@name,@address,@codp,@ville)'
  ...
  async updSite(params) {
    const query =
      'UPDATE `sites` SET `s_name`=@name,`s_address`=@address,`s_codp`=@codp,' + '`s_ville`=@ville WHERE `s_pk`=@sid'
  ...
  async getStationInfosById(cbpk) {
    const query =
      'SELECT `cb_pk`,`cb_id`,`cb_name`,`cb_pwd`,`registration_status`,`site` ' + 'FROM `charge_box` WHERE `cb_pk`=?'
  ...
  async getChargeBoxByName(ocppid, sid = null) {
    ...
    if (sid === null) {
      ...
      query = 'SELECT * FROM `charge_box` WHERE `site`=? AND `cb_id`=?'
      params = [sid, ocppid]
    }
    return await this.sqliteGet('getChargeBoxByName', query, params, true)
  }
  async getChargeBoxBootInfo(ocppid, sid = null) {
    ...
    if (sid === null) {
      ...
      query =
        'SELECT `ci`.* FROM `charge_box_infos` `ci` LEFT JOIN `charge_box` `cb` ON(`cb`.`cb_pk`=`ci`.`cb_pk`) WHERE `cb`.`site`=? AND `cb`.`cb_id`=?'
      params = [sid, ocppid]
    }
    return await this.sqliteGet('getChargeBoxBootInfo', query, params, true)
  }
  ...
  async getOwnerOrAdminIdTag(sid) {
    if (sid === null) {
      return await this.sqliteGet(
        'getOwnerOrAdminIdTag',
        'SELECT `idtag` FROM `rfid_tags` WHERE `site` IS NULL AND `isparent`=1',
        null,
        true
      )
    } else {
      return await this.sqliteGet(
        'getOwnerOrAdminIdTag',
        'SELECT `idtag` FROM `rfid_tags` WHERE `site`=? AND `isparent`=1',
        [sid],
        true
      )
    }
  ...
  async ajaxGetChargeurStatus(ocppid, sid = null) {
    ...
    if (sid === null) {
      ...
      query =
        'SELECT `online`,`last_heartbeat_ts`,`last_bootnotif_ts` FROM `charge_box` WHERE `site`=? AND `cb_id`=?'
      params = [sid, ocppid]
    }
    try {
      const stmt = this.db.prepare(query)
