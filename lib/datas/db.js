...
export default new class sqlite {
  ...
  getUserCredentials(usermail) {
    // Passport login
    const query = 'SELECT `user_pk`,`usermail`,`password`,`name`,`isAdmin`,`ownedSite` ' +
  ...
  async addSiteIdToken(sid) {
    const query = 'INSERT INTO `rfid_tags` (`idtag`,`site`,`rf_name`,`isparent`) VALUES (CONCAT(\'SADM\',HEX(RANDOMBLOB(6))),?,\'SITEADMIN\',1)'
  ...
  async getSitesOfUserById(uid) {
    const query = 'SELECT `SU`.`pk`,`SI`.`s_name` FROM `sites_users` `SU` ' +
  ...
  async addUser(params) {
    const query = 'INSERT INTO `users` (`usermail`,`password`,`name`,`isAdmin`,`ownedSite`,`idtag`) VALUES ' +
  ...
  // Get GlobalAdmin or SiteAdmin Special IDTAG
  async getAdminParentidTag(sid) {
    let query = ''
    if ( sid===null ) {
      query = 'SELECT `idtag` FROM `rfid_tags` WHERE `site` IS NULL AND `isparent`=1'
    } else {
      query = 'SELECT `idtag`,(SELECT `idtag` FROM `rfid_tags` WHERE `isparent`=1 AND `site` IS NULL) AS `parent` ' +
        'FROM `rfid_tags` WHERE `site`=' + sid + ' AND `isparent`=1'
    }
    return await this.sqliteGet('getAdminParentidTag', query, null, true)
  }
  // Get User IdTag and parent IdTag of ChargeBox
  async getUserParentidTag(params) {
    const query = 'SELECT `U`.`idtag`,(SELECT `idtag` FROM `rfid_tags` WHERE `isparent`=1 AND `site`=`SU`.`spk`) AS `parent` ' +
      'FROM `sites_users` `SU` ' +
      'LEFT JOIN `users` `U` ON (`U`.`user_pk`=`SU`.`upk`) ' +
      'LEFT JOIN `charge_box` `CB` ON(`CB`.`site`=`SU`.`spk`) ' +
      'WHERE `CB`.`cb_id`=@cbid AND `SU`.`upk`=@uid'
    return await this.sqliteGet('getUserParentidTag', query, params, true)
  }
  // Special
  ...
  async isUserOfChargeBox(ocppid, uid) {
    const query = 'SELECT `SU`.`uname` FROM `sites_users` `SU` ' +
      'LEFT JOIN `charge_box` `CB` ON(`CB`.`site`=`SU`.`spk`)' +
  ...
  async getChargeBoxConsLastTransac(ocppid, sid=null) {
    ...
    if (sid === null) {
      query = 'WITH tempt AS (SELECT `t`.*,`cb`.`online`,`rf`.`rf_name`,`u`.`name` AS `uname`,`u`.`usermail`,' +
        'ROW_NUMBER() OVER(PARTITION BY `t`.`con_pk` ORDER BY `t`.`t_pk` DESC) AS `nc` ' +
        'FROM `transactions` `t` ' +
  ...
  async getChargeBoxConsAndLastTransac(ocppid, sid=null) {
    let query = ''
    if (sid === null) {
      query = 'WITH tempt AS (SELECT `cs`.*,`rf`.`rf_name`,`u`.`name` AS `uname`,`u`.`usermail`,' +
      ...
      query = 'WITH tempt AS (SELECT `cs`.*,`rf`.`rf_name`,`su`.`uname`,`u`.`usermail`,' +
        '`t`.`t_pk`,`t`.`tsStart`,`t`.`tsStop`,`t`.`tsEnergy`,`t`.`tsPower`,`t`.`tsReason`,' +
        'ROW_NUMBER() OVER(PARTITION BY `con_pk` ORDER BY `t_pk` DESC) AS `nc` ' +
        'FROM `charge_box` `cb` ' +
        'LEFT JOIN `connector_status` `cs` ON (`cs`.`cb_pk`=`cb`.`cb_pk`) ' +
  ...
  async ajaxGetChargeBoxConsLastTransac(ocppid, sid=null) {
    let query = ''
    if (sid === null) {
      query = 'WITH tempt AS (SELECT `t`.*,`cs`.`pk`,`rf`.`rf_name`,`u`.`name` AS `uname`,`u`.`usermail`,' +
        'ROW_NUMBER() OVER(PARTITION BY `t`.`con_pk` ORDER BY `t`.`t_pk` DESC) AS `nc` ' +
        'FROM `transactions` `t` ' +
        'LEFT JOIN `connector_status` `cs` ON (`t`.`con_pk`=`cs`.`pk`) ' +
        'LEFT JOIN `charge_box` `cb` ON (`cs`.`cb_pk`=`cb`.`cb_pk`) ' +
      ...
      query = 'WITH tempt AS (SELECT `t`.*,`cs`.`pk`,`rf`.`rf_name`,`su`.`uname`,`u`.`usermail`,' +
        'ROW_NUMBER() OVER(PARTITION BY `t`.`con_pk` ORDER BY `t`.`t_pk` DESC) AS `nc` ' +
        'FROM `transactions` `t` ' +
        'LEFT JOIN `connector_status` `cs` ON (`t`.`con_pk`=`cs`.`pk`) ' +
        'LEFT JOIN `charge_box` `cb` ON (`cs`.`cb_pk`=`cb`.`cb_pk`) ' +
  ...
  async getDashboardCBStatus(sid=null) {
    let query = 'SELECT TOTAL(CASE WHEN `online`=1 THEN 1 ELSE 0 END) as `online`,' +
  ...
  async getDashboardSiteCbConnTransac(sid=null) {
    ...
    if (sid) {
      query = 'WITH tempt AS (' +
        'SELECT ROW_NUMBER() OVER(PARTITION BY COALESCE(`T`.`con_pk`,CONCAT(' + sid +',\'-\',COALESCE(`CB`.`cb_pk`,0))) ORDER BY `T`.`t_pk` DESC) AS `nc`,' +
        ...
        'LEFT JOIN `transactions` `T` ON(`T`.`con_pk`=`CS`.`pk`) ' +
        ') ' +
        'SELECT * FROM `tempt` WHERE `site`=' + sid +' AND `nc`=1 ORDER BY `cb_name`, `connector_id`'
      } else {
        query = 'WITH tempt AS (' +
