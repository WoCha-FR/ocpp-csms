...
export default new class sqlite {
  ...
  async addSiteIdToken(sid) {
    const query = 'INSERT INTO `rfid_tags` (`idtag`,`site`,`rf_name`,`isparent`) VALUES (CONCAT(\'SA\',HEX(RANDOMBLOB(6))),?,\'SITEADMIN\',1)'
  ...
  async getChargeBoxConsAndLastTransac(ocppid, sid=null) {
    let query = ''
    if (sid === null) {
      query = 'WITH tempt AS (SELECT `cb`.`online`,`cs`.*,`rf`.`rf_name`,`u`.`name` AS `uname`,`u`.`usermail`,' +
      ...
      query = 'WITH tempt AS (SELECT `cb`.`online`,`cs`.*,`rf`.`rf_name`,`su`.`uname`,`u`.`usermail`,' +
        '`t`.`t_pk`,`t`.`tsStart`,`t`.`tsStop`,`t`.`tsEnergy`,`t`.`tsPower`,`t`.`tsReason`,' +
        'ROW_NUMBER() OVER(PARTITION BY `con_pk` ORDER BY `t_pk` DESC) AS `nc` ' +
        'FROM `charge_box` `cb` ' +
        'LEFT JOIN `connector_status` `cs` ON (`cs`.`cb_pk`=`cb`.`cb_pk`) ' +
