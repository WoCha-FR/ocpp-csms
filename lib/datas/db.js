...
export default new class sqlite {
  ...
  async getAllSiteUsers(sid) {
    let query = 'SELECT NULL AS `pk`, `U`.`user_pk`,`U`.`name`,`U`.`usermail` FROM `users` `U` ' +
      'WHERE `U`.`ownedSite`=@sid ' +
      'UNION ' +
      'SELECT `S`.`pk`,`S`.`upk`,`S`.`uname`,`U`.`usermail` FROM `sites_users` `S` ' +
      'LEFT JOIN `users` `U` ON (`S`.`upk`=`U`.`user_pk`) WHERE `S`.`spk`=@sid ' +
      'ORDER BY `name` ASC'
    try {
      const stmt = this.db.prepare(query)
      const res = stmt.all({ sid: sid })
      return res
    } catch (err) {
  ...
  async getRfidTagInfosById(params) {
    ...
    if (params.sid) {
      query = query + ' AND `site`=' + params.sid
    }
    try {
      const stmt = this.db.prepare(query).bind(params.rf_pk)
      const res = stmt.get()
      return res
    } catch (err) {
  ...
  async getChargeBoxConfigKeys(ocppid) {
    let query = 'SELECT `cc`.`key`,`cc`.`value`,`cc`.`readonly` FROM `charge_box_conf` `cc` ' +
  ...
  async getChargeBoxConsLastTransac(ocppid, sid=null) {
    ...
    try {
      ...
      this.log.warn('getChargeBoxConsLastTransac: ' + err)
      return { error: new Error(`${err}`) }
    }
  }
  ...
  async getTransacGraphData(params) {
    let query = 'SELECT TIME(data_ts, \'unixepoch\') AS x, ROUND(CAST(energy AS real)/1000, 3) AS energy, ' +
      'ROUND(CAST(power AS real)/1000, 1) AS power ' +
      'FROM transactions_data WHERE t_pk=@tpk'
    try {
      let stmt = this.db.prepare(query)
      let res = stmt.all(params)
      return res
    } catch(err) {
      this.log.warn('getTransacGraphData: ' + err)
      return { error: new Error(`${err}`)}
    }
  }
  ...
  async getDashboardCBStatus(sid=null) {
    ...
    return this.getWithoutParamsUI('getDashboardCBStatus', query)
  }
  async getDashboardConnStatus(sid=null) {
    ...
    return this.getWithoutParamsUI('getDashboardConnStatus', query)
  }
  ...
  async updChargeBoxConfigKey(params) {
    ...
    return await this.runWithParams('updChargeBoxConfigKey', query, params)
  }
  ...
  async initTransactionData(params) {
    ...
    return await this.runWithParams('initTransactionData', query, params)
  }
  async addTransactionData(params) {
    ...
    return await this.runWithParams('addTransactionData', query, params)
  }
  async stopTransaction(params) {
    ...
    return await this.runWithParams('stopTransaction', query, params)
  }
/* CSMS - Autostart by OCPP / Charger Mode */
  ...
  /* GLOBAL FUNCTIONS
  async getAllWithoutParams(funcname, query) {

  }
  async getAllWithParams(funcname, query, params) {

  }
  */
  /* GET => Select that return the first row retrieved by the query
  *  return the row in object, undefined if no data or Error is thrown
  */
  async getWithParams(funcname, query, params) {
    try {
      let stmt = this.db.prepare(query)
      let res = stmt.get(params)
      return res // Values or undefined
  ...
  async getWithoutParamsUI(funcname, query) {
    try {
      let stmt = this.db.prepare(query)
      let res = stmt.get()
      return res
  ...
  *  return info.changes, info.lastInsertRowid or Error is thrown
  */
  async runWithParams(funcname, query, params) {
    try {
      let stmt = this.db.prepare(query)
      let res = stmt.run(params)
      return res
