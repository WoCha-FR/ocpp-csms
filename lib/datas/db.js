...
export default new class sqlite {
  ...
  async getAllStations(sid=null) {
    let query = ''
    if (sid) {
      query = 'SELECT cb_pk, cb_id, cb_name, registration_status, online ' +
        'FROM charge_box WHERE site=' + sid + ' ORDER BY cb_name'
    } else {
  ...
  async ajaxGetStationsStatus(sid=null) {
    let query = ''
    if (sid) {
      query = 'SELECT cb_pk, online, registration_status FROM charge_box WHERE site=' + sid
    } else {
      query = 'SELECT cb_pk, online FROM charge_box'
    }
    try {
      const stmt = this.db.prepare(query)
      const res = stmt.all()
