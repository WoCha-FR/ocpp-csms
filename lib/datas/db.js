...
export default new class sqlite {
  ...
  getUserCredentials(usermail) {
    // Passport login
    try {
      const stmt = this.db.prepare('SELECT `user_pk`, `password`, `name`, `isAdmin`, `ownedSite` FROM users WHERE usermail = ?')
  ...
  async getAllUsers() {
    try {
      const stmt = this.db.prepare('SELECT `user_pk`, `name`, `usermail`, `isAdmin`, `ownedSite` FROM users')
      const res = stmt.all()
      return res
    } catch(err) {
  ...
  async addUser(params) {
    let query = 'INSERT INTO users (`name`,`usermail`,`password`,`isAdmin`, `ownedSite`) VALUES ' +
  ...
  async updUser(params) {
    let query = 'UPDATE users SET `name`=@name, `usermail`=@uname, `email`=@umail, '
    if (params.upass) {
  ...
  async delRfidTag(params) {
    try {
      let stmt = this.db.prepare('DELETE FROM rfid_tags WHERE `rf_pk`=@rf_pk')
      let res = stmt.run(params)
      return res
    } catch (err) {
