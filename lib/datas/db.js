...
export default new class sqlite {
  ...
  async getAllChargers() {
    let query = 'SELECT CB.cb_pk, CB.cb_id, CB.cb_name, CB.registration_status, CB.online, US.name ' +
      'FROM charge_box CB LEFT JOIN users US on US.user_pk = CB.owner ' +
      'ORDER BY name, cb_name'
    try {
  ...
  async AdminAddChargeBox(params) {
    /* By Admin form */
    let query = 'INSERT INTO charge_box (cb_id,cb_name,cb_pwd,registration_status,owner) VALUES (@id,@name,@pwd,@regstatus,@owner)'
    let data = {
      id : params.ocppid,
      name: params.cbname,
  ...
  async updChargeBox(params) {
    let query = 'UPDATE charge_box SET cb_id=@id, cb_name=@name, cb_pwd=@pwd, registration_status=@regstatus, owner=@owner WHERE cb_pk=@cbpk'
    let data = {
      cbpk: params.cbpk,
      id: params.ocppid,
      name: params.cbname,
  ...
  async getChargeBoxInfosById(cbpk) {
    let query = 'SELECT cb_pk, cb_id, cb_name, cb_pwd, registration_status, owner ' +
