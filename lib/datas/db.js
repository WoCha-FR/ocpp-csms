...
export default new class sqlite {
  ...
  async ajaxGetChargeurConnsStatus(ocppid, sid=null) {
    let query = ''
    if (sid === null) {
      query = 'SELECT `cs`.`connector_id`,`cs`.`status`,`cs`.`status_ts`,`cs`.`energy`,`cb`.`online` FROM `connector_status` `cs` ' +
  ...
  async initTransactionData(params) {
    try {
      let query = 'INSERT INTO `transactions_data` (`t_pk`,`data_ts`) ' +
        'VALUES (@tpk,@ts)'
      let stmt = this.db.prepare(query)
      stmt.run(params)
    } catch(err) {
      this.log.warn('initTransactionData: ' + err)
      return false
    }
  }
  async addTransactionData(params) {
    try {
      let query = 'INSERT INTO `transactions_data` (`t_pk`,`data_ts`,`power`,`energy`) ' +
        'VALUES (@tpk,@ts,@power,@energy-(SELECT `meterStart` FROM `transactions` WHERE `t_pk`=@tpk))'
      let stmt = this.db.prepare(query)
      stmt.run(params)
    } catch(err) {
      this.log.warn('addTransactionData: ' + err)
      return false
    }
  }
