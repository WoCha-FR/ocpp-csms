...
export default new class sqlite {
  ...
  async getAllSites() {
    try {
      const stmt = this.db.prepare('SELECT `s_pk`,`s_name`,`s_address`,`s_codp`,`s_ville` FROM `sites` ORDER BY `s_name`')
      const res = stmt.all()
      return res
    } catch(err) {
  ...
  async getSiteUsers(sid) {
    let query = 'SELECT `S`.`pk`,`S`.`upk`,`S`.`uname`,`U`.`usermail` FROM `sites_users` `S` ' +
  ...
  async getUserSiteUserById(params) {
    let query = 'SELECT `SU`.`pk`,`SU`.`uname`,`U`.`usermail` FROM `sites_users` `SU` ' +
  ...
  async getChargeBoxRunningTransac(ocppid, sid=null) {
    let query = ''
    if (sid === null) {
      query = 'SELECT * FROM `transactions` WHERE `cb_id`=? AND `tsStop` IS NULL'
    } else {
      query = 'SELECT `t`.* FROM `transactions` `t` LEFT JOIN `charge_box` `cb` ON (`cb`.`cb_id`=`t`.`cb_id`) ' +
        'WHERE `cb`.`site`=' + sid + ' AND `t`.`cb_id`=? AND `tsStop` IS NULL'
    }
    try {
      const stmt = this.db.prepare(query)
      const res = stmt.all(ocppid)
      ...
      this.log.warn('getChargeBoxRunningTransac: ' + err)
      return { error: new Error(`${err}`) }
    }
  }
  async updConnectorPerso(params) {
    ...
    if (params.sid===null) {
      query = 'UPDATE `connector_status` SET `cname`=@cname,`maxPower`=@cmax,`format`=@cfor,`type`=@ctyp ' +
        'WHERE `cb_id`=@cbid AND `connector_id`=@cid'
    } else {
      query = 'UPDATE `connector_status` SET `cname`=@cname,`maxPower`=@cmax,`format`=@cfor,`type`=@ctyp ' +
        'WHERE (SELECT `site` FROM `charge_box` WHERE `cb_id`=@cbid)=@sid ' +
        'AND `cb_id`=@cbid AND `connector_id`=@cid'
    }
    try {
      let stmt = this.db.prepare(query)
      let res = stmt.run(params)
      ...
      this.log.warn('updConnectorPerso: ' + err)
      return { error: new Error(`${err}`)}
    }
  }
  /* WEBUI - AJAX */
  async ajaxGetStationsStatus(sid=null) {
    let query = ''
    if (sid) {
      query = 'SELECT `cb_pk`,`registration_status`,`online` FROM `charge_box` WHERE `site`=' + sid
    } else {
      query = 'SELECT `cb_pk`,`registration_status`,`online` FROM `charge_box`'
    }
    try {
      const stmt = this.db.prepare(query)
      const res = stmt.all()
  ...
  async ajaxGetChargeurStatus(ocppid, sid=null) {
    let query = ''
    if (sid === null) {
      query = 'SELECT `online` FROM `charge_box` WHERE `cb_id`=?'
    } else {
      query = 'SELECT `online` FROM `charge_box` WHERE `site=`' + sid + ' AND `cb_id`=?'
    }
    try {
      const stmt = this.db.prepare(query)
      const res = stmt.get(ocppid)
  ...
  async ajaxGetChargeurConnsStatus(ocppid, sid=null) {
    let query = ''
    if (sid === null) {
      query = 'SELECT `connector_id`,`status`,`status_ts`,`energy` FROM `connector_status` WHERE `cb_id`=?'
    } else {
      query = 'SELECT `cs`.`connector_id`,`cs`.`status`,`cs`.`status_ts`,`cs`.`energy` FROM `connector_status` `cs` ' +
        'LEFT JOIN `charge_box` `cb` ON(`cb`.`cb_id`=`cs`.`cb_id`) ' +
        'WHERE `cb`.`site`=' + sid + ' AND `cs`.`cb_id`=?'
    }
    try {
      const stmt = this.db.prepare(query)
  ...
  updConnectorStatus(ocppid, params) {
    ...
    let query = 'INSERT INTO `connector_status` (' +
      ...
      ') VALUES ' +
      '((SELECT `cb_pk` FROM `charge_box` WHERE `cb_id`=@cb_id),@conn_id,@stat,@stat_ts,@ec,@ei,@vei,@vec) ' +
      'ON CONFLICT(`cb_pk`,`connector_id`) DO UPDATE SET ' +
      ...
      '`vendor_error_code`=excluded.vendor_error_code WHERE `cb_pk`=(SELECT `cb_pk` FROM `charge_box` WHERE `cb_id`=@cb_id) ' +
      'AND `connector_id`=@conn_id;'
    // Data
  ...
  async updMeterValues(params) {
    try {
      let stmt = this.db.prepare('UPDATE `connector_status` SET `energy`=@energy WHERE ' +
        '`cb_id`=(SELECT `cb_pk` FROM `charge_box` WHERE `cb_id`=@cbid) AND `connector_id`=@cnid')
      stmt.run(params)
      return true
    } catch(err) {
  ...
  async updChargeBoxConfigKey(params) {
    try {
      let stmt = this.db.prepare('UPDATE `charge_box_conf` SET ' +
        '`value`=@value WHERE `cb_pk`=(SELECT `cb_pk` FROM `charge_box` WHERE `cb_id`=@ocppName) AND `key`=@key')
      let res = stmt.run(params)
      return res
    } catch(err) {
  ...
  async addTransaction(params) {
    try {
      /* Select Con_PK */
      let query = 'SELECT `con_pk` FROM `connector_status` WHERE `connector_id`=@conid AND '+
      ...
      try {
        let query = 'INSERT INTO `transactions` (`t_pk`,`con_pk`,`IdToken`,`meterStart`,`tsStart`,`tsStatus`) ' +
          'VALUES (@tpk,' + con_data.con_pk + ',@idtag,@meter,@start,@status)'
        let stmt = this.db.prepare(query)
        stmt.run(params)
        return true
      } catch(err) {
  ...
  async getAuthorizeResponse(ocppid, idTag) {
   let query = 'SELECT `CB`.`expire`,`CB`.`blocked` FROM `charge_box_rfids` `CB` ' +
   'LEFT JOIN `rfid_tags` `RF` ON (`CB`.`rf_pk`=`RF`.`rf_pk`) WHERE `CB`.`cb_id`=? AND `RF`.`idtag`=?'
   try {
    let stmt = this.db.prepare(query)
