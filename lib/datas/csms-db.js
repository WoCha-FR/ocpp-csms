...
export default new class CSMSDB {
  ...
  async generateTransactionId() {
    ...
    // Query
    try {
      let stmt = sqlite.db.prepare('SELECT MAX(`t_pk`) AS `max` FROM `transactions` WHERE `t_pk` > ?')
  ...
  async addTransactionData(params) {
    const query = 'INSERT INTO `transactions_data` (`t_pk`,`data_ts`,`power`,`energy`,`ampl1`,`ampl2`,`ampl3`,`ampoffer`) ' +
      'VALUES (@tpk,@ts,@power,@energy-(SELECT `meterStart` FROM `transactions` WHERE `t_pk`=@tpk),@ampl1,@ampl2,@ampl3,@ampoffer)'
