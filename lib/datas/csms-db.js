...
class CSMSDB {
  ...
  async AutoAddChargeBox(params) {
    // Delegate to the main db module to avoid code duplication
    return await sqlite.AutoAddChargeBox(params)
  }
  ...
  addToLog(params) {
    ...
    let stmt = sqlite.db.prepare(
      'INSERT INTO `charge_box_log` (`cb_pk`,`timestamp`,`direction`,`type`,`method`,`message`) ' +
  ...
  updConnectorStatus(ocppid, params) {
    let queryS =
      'SELECT `pk` FROM `connector_status` WHERE ' +
      '`cb_pk`=(SELECT `cb_pk` FROM `charge_box` WHERE `cb_id`=@cb_id) AND `connector_id`=@conn_id'
    // Update Query
    let queryU =
      'UPDATE `connector_status` SET `status`=@stat,`status_ts`=@stat_ts,`error_code`=@ec,`error_info`=@ei,' +
  ...
  async getAuthorizeResponse(ocppid, idTag) {
    ...
    if (idTag.startsWith('SADM')) {
      ...
      try {
        ...
        if (result === undefined) {
          const ret = { ocppresp: { status: 'Invalid', expiryDate: expire }, idtaginfo: { rfpk: null, upk: null } }
          return ret
        }
        const ret = { ocppresp: { status: 'Accepted', expiryDate: expire }, idtaginfo: { rfpk: result.rf_pk, upk: 0 } }
        return ret
      } catch (err) {
    ...
    if (idTag.startsWith('WUSR')) {
      ...
      try {
        ...
        const ret = { ocppresp: { status: 'Accepted', expiryDate: expire }, idtaginfo: { rfpk: null, upk: result.user_pk } }
        return ret
      } catch (err) {
  ...
  saveAuthorizeResponse(params) {
    const query =
      'INSERT INTO `idtags_logs` (`cb_pk`,`ts`,`idtag`,`resp`,`method`) ' +
  ...
  async generateTransactionId() {
    ...
    const daynum =
      (Date.UTC(today.getFullYear(), today.getMonth(), today.getDate()) - Date.UTC(today.getFullYear(), 0, 0)) / 86400000
  ...
  async addTransaction(params) {
    try {
      /* Select Con_PK */
      const query =
        'SELECT `pk` FROM `connector_status` WHERE `connector_id`=@conid AND ' +
      ...
      try {
        const query =
          'INSERT INTO `transactions` (`t_pk`,`con_pk`,`rfpk`,`upk`,`IdToken`,`meterStart`,`tsStart`,`tsStatus`) ' +
          'VALUES (@tpk,' +
          con_data.pk +
          ',@rfpk,@upk,@idtag,@meter,@start,@status)'
        const stmt = sqlite.db.prepare(query)
        stmt.run(params)
        return true
      } catch (err) {
  ...
  async stopTransaction(params) {
    const query =
      'UPDATE `transactions` SET `meterStop`=@meter,`tsStop`=@stop,' +
      '`tsReason`=@reason,`tsEnergy`=@meter-`meterStart`,`tsPower`=@power ' +
      'WHERE `t_pk`=@tpk AND `tsStop` IS NULL'
  ...
  async addTransactionData(params) {
    const query =
      'INSERT INTO `transactions_data` (`t_pk`,`data_ts`,`energy`,`power`,`poffer`,`ampl1`,`ampl2`,`ampl3`,`ampoffer`,`context`) ' +
  ...
  updMeterValues(params) {
    try {
      let stmt = sqlite.db.prepare(
        'UPDATE `connector_status` SET `energy`=@energy WHERE ' +
  ...
  updTransactionMeterValues(params) {
    try {
      let stmt = sqlite.db.prepare(
        'UPDATE `transactions` SET `tsEnergy`=@energy-`meterStart`, `tsPower`=@power WHERE `t_pk`=@tpk'
      )
      stmt.run(params)
      return true
    } catch (err) {
  ...
  async saveChargeBoxConfigKeys(ocppid, config) {
    try {
      ...
      let insertAll = sqlite.db.transaction((keys) => {
        for (const k of keys) {
          stmt.run({ id: ocppid, key: k.key, value: k.value, readonly: k.readonly === true ? 1 : 0 })
        }
      })
      // Execute
  ...
  async updChargeBoxConfigKey(params) {
    const query =
      'UPDATE `charge_box_conf` SET `value`=@value ' +
