...
export default new class CSMSDB {
  ...
  async getAuthorizeResponse(ocppid, idTag) {
    ...
    try {
      ...
      if (res === undefined) {
        return { status: 'Invalid', expiryDate: new Date(Date.now()).toISOString() }
      } else if (res.blocked === 1) {
        return { status: 'Blocked', expiryDate: new Date(Date.now()).toISOString() }
      } else if (res.expire !== null && res.expire < now) {
        return { status: 'Expired', expiryDate: new Date(Date.now()).toISOString() }
      } else {
        let expiry
        if (res.expire !== null) {
