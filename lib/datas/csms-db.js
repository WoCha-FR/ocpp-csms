...
export default new class CSMSDB {
  ...
  async getAuthorizeResponseOLD(ocppid, idTag) {
    const expire = new Date(Date.now()).toISOString()
    // Is Admin TAG ?
    if (idTag.startsWith('SADM')) {
      const qadmin = 'SELECT `RT`.`rf_pk`,' +
        '(SELECT `idtag` FROM `rfid_tags` WHERE `isparent`=1 AND `site` IS NULL) AS `parent` ' +
        'FROM `rfid_tags` `RT` ' +
      ...
      try {
        ...
        const res = stmt.get(ocppid,idTag)
        if (res === undefined) {
          return { status: 'Invalid', expiryDate: expire }
        }
        if (res.parent == idTag) {
          return { status: 'Accepted', expiryDate: expire }
        } else {
          return { status: 'Accepted', expiryDate: expire, parentIdTag: res.parent }
        }
      } catch(err) {
        sqlite.WriteLog('getAuthorizeResponse: ' + err, 'WARN')
        return { status: 'Invalid', expiryDate: expire }
      }
    }
    // Is User TAG ?
    if (idTag.startsWith('WUSR')) {
      const quser = 'SELECT `U`.`idtag`,' +
        '(SELECT `idtag` FROM `rfid_tags` WHERE `isparent`=1 AND `site`=`SU`.`spk`) AS `parent` ' +
        'FROM sites_users `SU` ' +
    ...
    // Real RFID tag
    const query = 'SELECT `RT`.`blocked`, `RT`.`expire`,`RT`.`isparent`,' +
      '(SELECT `idtag` FROM `rfid_tags` WHERE `isparent`=1 AND `site`=`CB`.`site`) AS `parent` ' +
    ...
    try {
      ...
      if (res === undefined) {
        ...
        return { status: 'Blocked', expiryDate: expire }
      } else if (res.expire !== null && res.expire < now) {
        return { status: 'Expired', expiryDate: expire }
      } else {
        let expiry
        if (res.expire !== null) {
  ...
  async getAuthorizeResponse(ocppid, idTag) {
    ...
    if (idTag.startsWith('SADM')) {
      ...
      try {
        ...
        if (result.parent == idTag) {
          ...
          const ret = {
            ocppresp: { status: 'Accepted', expiryDate: expire, parentIdTag: result.parent },
            idtaginfo: { rfpk: result.rf_pk , upk: 0 }
          }
          return ret
        }
    ...
    // Real TAG
    const query = 'SELECT `RT`.`rf_pk`,`RT`.`user`,`RT`.`blocked`,`RT`.`expire`,`RT`.`isparent`,' +
      '(SELECT `idtag` FROM `rfid_tags` WHERE `isparent`=1 AND `site`=`CB`.`site`) AS `parent` ' +
      'FROM `rfid_tags` AS `RT` ' +
      'LEFT JOIN `charge_box` `CB` ON (`RT`.`site`=`CB`.`site`) ' +
      'WHERE (`CB`.`cb_id`=? OR `RT`.`site` IS NULL) AND `RT`.`idtag`=?'
  ...
  async addTransactionData(params) {
    const query = 'INSERT INTO `transactions_data` (`t_pk`,`data_ts`,`energy`,`power`,`poffer`,`ampl1`,`ampl2`,`ampl3`,`ampoffer`) ' +
      'VALUES (@tpk,@ts,@energy-(SELECT `meterStart` FROM `transactions` WHERE `t_pk`=@tpk),@power,@poffer,@ampl1,@ampl2,@ampl3,@ampoffer)'
