...
export default new class UserDB {
  ...
  async updUserNotifs(params) {
    const query = 'UPDATE `user_notifs` SET `lng`=@lng, `mailEnabled`=@eml, `pushEnabled`=@psh,' +
      '`pushuser`=@pshu, `pushtokn`=@psht, `cbOnline`=@cbon, `cbOffline`=@cboff,' +
      '`conFaulted`=@conf, `conAvailable`=@cona, `conUnavailable`=@conu,' +
      '`chgStart`=@chgd, `chgStop`=@chgf, `chgEvent`=@chge ' +
      'WHERE `user_pk`=@uid'
  ...
  async getAdminToNotifyCB(event, cb_id) {
    let query = 'SELECT `U`.`usermail`,`U`.`name`,`U`.`isAdmin`,`UN`.`lng`,`UN`.`mailEnabled`,`UN`.`pushEnabled`,`UN`.`pushuser`,' +
